...
std::vector<MODULEENTRY32W> ProcessHacker::EnumerateModules()
{
	...
	if (aProcArchitecture == ARCH_AMD64)
		flags |= TH32CS_SNAPMODULE32;

	HANDLE th32 = CreateToolhelp32Snapshot(flags, GetProcessId(hOpenProcess));

	if (th32 && th32 != INVALID_HANDLE_VALUE)
	{
...
WritenInfo ProcessHacker::SetMemoryLocation(LPVOID address, LPVOID data, SIZE_T dataSize)
{
	...
	DWORD lastProtection = SetMemoryProtection(address, dataSize, PAGE_EXECUTE_READWRITE);

	if (hOpenProcess == hCurrentProcess)
	{

	}

	info.hasBeenWriten = WriteProcessMemory(hOpenProcess, address, data, dataSize, &info.totalBytesWriten);
	SetMemoryProtection(address, dataSize, lastProtection);
...
WritenInfo ProcessHacker::SetRelativeMemoryLocation(LPVOID address, long long int offset, LPVOID data, SIZE_T dataSize)
{
	return SetMemoryLocation((LPVOID)(((char*)address) + offset), data, dataSize);
}

ReadInfo ProcessHacker::ReadRelativeMemoryLocation(LPVOID address, long long int offset, SIZE_T readLength)
{
...
{
	if (hOpenProcess != GetCurrentProcess())
	{
...
ReadInfo ProcessHacker::ReadMultiDimensionPointerMemory(LPVOID address, long long int* offsets, SIZE_T offsetsLength, SIZE_T readLength)
{
	return ReadMemoryLocation(MultiDimensionPointerResolver(address, offsets, offsetsLength), readLength);
}

WritenInfo ProcessHacker::SetMultiDimensionPointerMemory(LPVOID address, long long int* offsets, SIZE_T offsetsLength, LPVOID data, SIZE_T writeLength)
{
	return SetMemoryLocation(MultiDimensionPointerResolver(address, offsets, offsetsLength), data, writeLength);
}

LPVOID ProcessHacker::MultiDimensionPointerResolver(LPVOID initialAddress, long long int* offsets, SIZE_T offsetsLength)
{
...
WritenInfo ProcessHacker::PatchRelativeMemoryRegion(LPVOID address, long long int offset, LPVOID data, SIZE_T dataSize)
{
	return PatchMemoryRegion((LPVOID)(((char*)address) + offset), data, dataSize);
}

WritenInfo ProcessHacker::PatchMultiDimensionPointerRegion(LPVOID address, long long int* offsets, SIZE_T offsetsLength, LPVOID data, SIZE_T dataSize)
{
...
WritenInfo ProcessHacker::RemoveRelativeMemoryRegionOperation(LPVOID address, long long int offset, SIZE_T regionSize)
{
