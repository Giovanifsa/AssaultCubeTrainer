...
public:
	...
	WritenInfo SetMultiDimensionPointerMemory(LPVOID address, long long int* offsets, SIZE_T offsetsLength, LPVOID data, SIZE_T writeLength);
	WritenInfo SetRelativeMemoryLocation(LPVOID address, long long int offset, LPVOID data, SIZE_T dataSize);
	...
	WritenInfo PatchRelativeMemoryRegion(LPVOID address, long long int offset, LPVOID data, SIZE_T dataSize);
	WritenInfo PatchMultiDimensionPointerRegion(LPVOID address, long long int* offsets, SIZE_T offsetsLength, LPVOID data, SIZE_T dataSize);

	WritenInfo RemoveMemoryRegionOperation(LPVOID address, SIZE_T regionSize);
	WritenInfo RemoveRelativeMemoryRegionOperation(LPVOID address, long long int offset, SIZE_T regionSize);
	...
	ReadInfo ReadMultiDimensionPointerMemory(LPVOID address, long long int* offsets, SIZE_T offsetsLength, SIZE_T readLength);
	ReadInfo ReadRelativeMemoryLocation(LPVOID address, long long int offset, SIZE_T readLength);

	MODULEENTRY32W FindModuleByName(const WCHAR* moduleName);

	DWORD ChangeProcessAccess(DWORD desiredAccess);
	LPVOID MultiDimensionPointerResolver(LPVOID initialAddress, long long int* offsets, SIZE_T offsetsLength);
